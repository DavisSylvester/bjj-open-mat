...
describe("OpenMatFacade", () => {
  it("create denormalizes gym name + location", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-1", nullGeocoder);
  ...
  it("non-owner submission is live but unverified", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-x", nullGeocoder);
  ...
  it("gym owner submission to own gym is verified", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-y", nullGeocoder);
    const created = await facade.create("owner-1", "gym_owner", { gymId: "g-1", title: "Fri", startTime: "19:00", endTime: "21:00" });
  ...
  it("admin submission is verified", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-z", nullGeocoder);
  ...
  it("newGym creates an unverified ownerless gym", async () => {
    const d = deps();
    let n = 0;
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => `id-${++n}`, nullGeocoder);
  ...
  it("verify sets verified=true for the gym owner; rejects others", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-v", nullGeocoder);
    await facade.create("owner-1", "gym_owner", { gymId: "g-1", title: "Fri", startTime: "19:00", endTime: "21:00" });
  ...
  it("verify rejects not_found for a missing mat", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-m", nullGeocoder);
  ...
  it("setHidden(false) returns a session to live", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-u", nullGeocoder);
    await facade.create("owner-1", "gym_owner", { gymId: "g-1", title: "Fri", startTime: "19:00", endTime: "21:00" });
  ...
  it("admin can hide any session", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-h", nullGeocoder);
    await facade.create("owner-1", "gym_owner", { gymId: "g-1", title: "Fri", startTime: "19:00", endTime: "21:00" });
  ...
  it("admin can update a session at a gym they do not own", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-adm", nullGeocoder);
  ...
  it("non-owner non-admin update is rejected with forbidden", async () => {
    const d = deps();
    const facade = new OpenMatFacade(d.matRepo, d.gymRepo, d.rsvpRepo, () => "om-rej", nullGeocoder);
    await facade.create("owner-1", "gym_owner", { gymId: "g-1", title: "Fri", startTime: "19:00", endTime: "21:00" });
    await expect(facade.update("intruder", "practitioner", "om-rej", { title: "Hacked" })).rejects.toMatchObject({ code: "forbidden" });
  });

  it("newGym with postalCode geocodes location via injected geocoder", async () => {
    const d = deps();
    let n = 0;
    ...
    const created = await facade.create("stranger", "practitioner", {
      newGym: { name: "Texas BJJ", address: "1 Main St", postalCode: "75495" },
      title: "Sat Open Mat",
      startTime: "10:00",
      endTime: "12:00",
    });
    expect(created.latitude).toBe(33.42);
